...
void Web::paint(QWidget *ctx) {
    ...
    QPoint endPoints[] = {
        p1,
        {ctx->width() / scaleParamWidth, 0},
        {ctx->width() / scaleParamWidth * delParam, 0},
        p2,
    ...
    for (int i = 0; i < 10; i ++) {
        ...
        QPoint localP2(ctx->width() + (line2.x1() - line2.x2()) / scaleParam * i, 0 + (line2.y1() - line2.y2()) / scaleParam * i);
...
int Web::getAngle(const QPoint& p1, const QPoint& p2) {
    qreal dx = p2.x() - p1.x();
    qreal dy = p2.y() - p1.y();
    qreal angle = qAtan2(dy, dx) * pi / M_PI;
    return static_cast<int>(angle * scale);
}
