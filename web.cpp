...
void Web::CreateWeb(const QRect &windowRect, const QPoint &cursorPosition) {
  ...
  webLines_.append(QLine(QPoint(centerX, centerY), QPoint(windowRect.width() / 2, 0)));
  webLines_.append(QLine(QPoint(centerX, centerY), QPoint(windowRect.width() / 2, windowRect.height())));
}
